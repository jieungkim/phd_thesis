...
\le_{\termnum} : \termnum \rightarrow \termnum \rightarrow \propkwd & \mbox{(binary relation for ballot values)}\\
\funwitness : \le_{\termnum}  \rightarrow  \isquorums  \rightarrow \set{\nodeid}  \rightarrow \termnum \rightarrow \set{\packet_\witness} \rightarrow \dstate \rightarrow \witness 
...
$
\begin{array}{llll}
\getvbnp : \packet \rightarrow \termnum  & \mbox{(voted ballot value of  packet}\ \packet\mbox{)} \\
\getdsvalue : \packet \rightarrow \dstate  & \mbox{(stored value of  packet}\ \packet\mbox{)} \\
...
$$
\begin{array}{c}
 \funwitness(\le_{\termnum},~\isquorums,~\accsset, ~\termnum, ~pkts) = \witness_{\termnum'} \ \cons \ (\termnum, ~\dstate', ~accs, ~\accsset, ~pkts') \rightarrow\\
...
src \in accs~\wedge~\getvbn(\msg) <_{\termnum} \termnum)) \\ 

(\exists p\ p_{\witness}.~p \in  pkt'~\wedge~p_{\witness} \in pkt~\wedge\\
\projpkt(p_{\witness}) = p~\wedge~~\getwitness(p_{\witness}) = \mathrm{Some}\ \witness_{\termnum'}~\wedge~
\getvbnp(p) = \termnum'~\wedge\\
(\forall p'.\ p' \in pkt' \rightarrow \getvbnp(p') \le_{\termnum} \getvbnp(p))~\wedge\\
( \getvbnp(p) = \bot \rightarrow \getdsvalue(p) = \bot~\wedge~ \dstate' = \dstate)~\wedge \\
( \getvbnp(p) \neq \bot \rightarrow \dstate' = \getdsvalue(p)) \\
\end{array}
$$
