...
\pref{sec:specs-for-leader-based-system}. Our network is defined as a list of
network events, and the key definitions are found in \pref{fig:net-defs}. A
...
low-level instantiation of $\packet$ from \pref{fig:basic-state}.
...
reasoning and refinement. \ignore{Projection functions are defined for narrowing the
view of the network log to only the events from a given channel, to focus on
only the relevant history.} Assigning different channel identifiers for each
...
with all intermediate events. \ignore{(including events not actually observable by the
...
relevant forms of network failure, \ignore{(which can be specified per protocol),} and
...
Two example network reductions are shown in \pref{fig:network-reduction}. In
...
\label{subsubsec:building-global-state-transition-systems}
}
