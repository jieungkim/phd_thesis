...
\ignore{In either case, Raft protocol returns success only to a
...
the template for the linearizability proof. \jiyong{A lot of claims are made
without a concret evidence and I'll need to revisit this part again.}}
...
%$G_{ldr}$ acquires the lock. And releases the old one? Or part of $G_{op}$?
%$G_{op}$ manipulates the state.
%Requires stronger assumptions about network failure conditions than the other
%systems we've considered.
%In particular, messages must be eventually delivered, and messages from the
%same node must arrive in the order they were sent.
%Similar to the semantics provided by TCP.
%We handle this by adding additional constraints on the network oracle.
