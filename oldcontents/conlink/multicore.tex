...
We view the multicore machine as a set of per-CPU machines~\jieung{ccal}.
...
We first introduce concurrent semantics, an abstract model of x86 multicore machine model in CertiKOS verification\jieung{need cite}, the bases of our verification framework. 

\ignore{Multicore platform is purely concurrent environment that fixed number of CPUs can run their own programs with any kinds of interleaving. 
Those complexities are too obsolete to be handled in the verification of the whole system. 
In this sense, 

CompCert related things, 

Figure~\jieung{need to make figure} shows how we provide the linking in between 
Key points that the multicore linking need to provide are 1) showing that multiple per-CPU machines can be combined into a single multi-core machine model, and 2) .}
...
\fbox{abstract hardware setting, abstract command definitions:}
...
\primitiveid & : & \set{ id : \ztype ~\vert~ id > 0} & \mbox{(Identifier for pritimives)} \\
...
\fbox{abstract hardware semantics:}
...
\inferrule[Log Get Atom \nulllist]
{\ }
{\loggetatom{\nulllist}{eid}{\nulllist}}

\inferrule[Log Get Atom add]
{\loggetatom{l}{eid}{l'}\\
eid = eid' \\
(ev = \mathrm{EATOMIC} \ \_ \ eid' \vee ev = \mathrm{EACQ} \ \_ \ eid'  \vee ev = \mathrm{EACQ} \ \_ \ eid')}
{\loggetatom{(\listcons{ev}{l})}{eid}{(\listcons{ev}{l'})}}

\inferrule[Log Get Atom noadd]
{\loggetatom{l}{eid}{l'} \\
((ev = \mathrm{EATOMIC} \ \_ \ eid' \vee ev = \mathrm{EACQ} \ \_ \ eid'  \vee ev = \mathrm{EACQ} \ \_ \ eid') \wedge eid \neq eid') \vee 
ev = \_)}
{\loggetatom{(\listcons{ev}{l})}{eid}{l'}}

\end{mathpar}
}
%%%% no need

\fbox{properties of abstract hardware semantics:}
...
\inferrule[\nulllist]
{ \ }
{ \calowner{\nulllist}{id}{(\ofreestate{\optionnone})}}
...
{ \calowner{\listcons{ev}{l'}}{id}{(\ownstate{from})}}
...
{ \calowner{\listcons{ev}{l'}}{id}{(\ofreestate{(\optionsome \ d)})}}


\inferrule[others]
{\calowner{l'}{id}{o} \\
(e \neq \acqev{\_}{\_} \wedge e \neq \relev{\_}{\_}{\_}) \vee 
((e = \acqev{\_}{id'} \vee e \neq \relev{\_}{id'}{\_}) \wedge id' \neq id) \vee
(e = \acqev{from}{id'} \wedge id' = id \wedge o \neq \ofreestate{\_})\vee
(e =\relev{from}{id'}{d} \wedge id' = id \wedge (o \neq \ownstate{i} \vee (o = \ownstate{i}  \wedge i \neq from))) 
}
{ \calowner{\listcons{ev}{l'}}{id}{(\ofreestate{(\optionsome \ d)})}}
...
\fbox{hardware local step: $\ztype \rightarrow \localviewkwd \rightarrow \localview \rightarrow \mcprop$}
\begin{mathpar}
\inferrule[acqrule]
...
\caption{Abstract Semantics for Local Evaluations - Local Evaluation in Each Core}
\label{fig:hardware-local-abstract-definition}
\end{figure}
...
\fbox{getter and setter function:} 
...
\fbox{hardware step: $(start: \ztype) : \hstatekwd \rightarrow \cctracekwd \rightarrow \hstatekwd \rightarrow \mcprop$}
\begin{mathpar}
...
The main idea of multicore machine is providing one more logical hardware, which we call a hardware scheduler. 
...
\fbox{oracle step: $(oracle:\mcoracle{\coreset}{\mclog}{\mcevent}) :  \mcstatekwd \rightarrow \cctracekwd \rightarrow \mcstatekwd \rightarrow \mcprop$}
...
\caption{Abstract Syntax and Semantics for Oracle Machine}
\label{fig:oracle-step-definition}
\end{figure}

The previous model is a purely non-deterministic machine model. 
...
\fbox{env step: ${}_{[A : \set{\ztype}]} (oracle:\mcoracle{A}{\mclog}{\mcevent}) :  \mcstatekwd_{[A]} \rightarrow \cctracekwd \rightarrow \mcstatekwd_{[A]} \rightarrow \mcprop$}
\begin{mathpar}
...
\fbox{single step: ${}_{[curid : \ztype]} (oracle : \mcoracle{\set{curid}}{\mclog}{\mcevent}) :  \singlestatekwd \rightarrow \cctracekwd \rightarrow \singlestatekwd \rightarrow \mcprop$}
...
\caption{Abstract Syntax and Semantics for Single Step Machine}
\label{fig:single-step-definition}
\end{figure}
...
\label{subsubsec:optimization-mc}


The environmental context is defined as an single-step operational way. 
...
\jieung{Figure should be better}
...
${}_{[curid : \ztype]} (oracle : \mcoracle{\set{curid}}{\mclog}{\mcevent}) :  \singlestatekwd \rightarrow \cctracekwd \rightarrow \singlestatekwd \rightarrow \mcprop$}
...
\caption{Abstract Syntax and Semantics for Single Big Step Machine}
\label{fig:single-big-step-definition}
\end{figure}
...
\caption{Abstract Syntax and Semantics for Single Big Two Step Machine}
\label{fig:single-big-two-step-definition}
\end{figure}


\begin{figure}
\fbox{single local view:}
$
\begin{array}{lll}
...
\fbox{single local step: $\ztype \rightarrow \singlelocalviewkwd \rightarrow \singlelocalviewkwd \rightarrow \mcprop$}
...
\fbox{single split step:
$
{}_{[curid : \ztype]} (oracle : \mcoracle{\set{curid}}{\mclog}{\mcevent}) :  \srstatekwd \rightarrow \cctracekwd \rightarrow \srstatekwd \rightarrow \mcprop
$}
...
\caption{Abstract Syntax and Semantics for Split Step Machine}
\label{fig:split-step-definition}
\end{figure}


\begin{figure}
\fbox{single reorder step:
$
{}_{[curid : \ztype]} (oracle : \mcoracle{\set{curid}}{\mclog}{\mclog}) :  \srstatekwd \rightarrow \cctracekwd \rightarrow \srstatekwd \rightarrow \mcprop
$}
...
\caption{Abstract Syntax and Semantics for Reorder Step Machine}
\label{fig:reorder-step-definition}
\end{figure}



\begin{figure}
\fbox{separate log and sp state:}
...
\fbox{separate log step:
$ \ztype \rightarrow \ztype \rightarrow \localviewkwd \rightarrow \privatestate \rightarrow \separateeventkwd \rightarrow \mcprop 
$}
\begin{mathpar}
\inferrule[acquire]
{\programcounter{ps}{\acqsharedcmd{id}}\\
...
\fbox{auxiliary functions for the differences of the logs:}
...
\caption{Abstract Syntax and Semantics for Single Separate Machine}
\label{fig:separte-step-definition}
\end{figure}
