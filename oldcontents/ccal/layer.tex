...
\label{sec:layer}
\ronghui{
An outline for the technical parts:
\begin{itemize}
\item A brief introduction to the sequential setting.
What is a layer? The main job of building layer is to
turn memory bytes into abstract state and turn 
function implementation into its spec (\ie, primitives).
Each step is either execute the instruction
or invoke a primitive.
\item Extensions to support concurrency.
1) log for concurrent part. The spec of concurrent primitive
is generating the corresponding event
and the shared abstract state can be calculated by the log;
2) environment context to capture the behaviors of the context
CPU and threads;
3) introduce an environmental step, which queries the oracle.
\item Partial machine and composition.
Thanks to the environmental step, we could introduce
partial machine, which only focus on a set of CPUs or threads.
When composing them together, we can only compose
the executions with the same log.
\item Multicore part.
1) Push/pull model, lift the bytes update to the log
level, but unsafe to execute;
2) Introduce spin-lock as safe push/pull;
3) Using push/pull to verify a shared queue object.
\item Multithreads part.
1) Using shared queue to prove the scheduling primitives
at the assembly level;
2) Lift the primitive to the C level
and introduce a new kind of environmental step
and composition.
3) Verify the queuing lock based on this thread-local machine.
\end{itemize}
}
...
\inferrule{
  ...
  L_1 \vdash_{R \circ S} M \oplus N : L_3
}
\end{mathpar}
\vspace{-5pt}
}%
...
%We say that $M_1 \refines_f M_2$ if there exists a relation
