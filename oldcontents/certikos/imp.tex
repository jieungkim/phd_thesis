...
\paragraph{Proof effort and the cost of change} 
We take the certified sequential mCertiKOS kernel~\cite{dscal15},
and extend the kernel with various features such as dynamic memory
management, container support for controlling resource consumption,
Intel hardware virtualization support, shared memory IPC, single-copy
synchronous IPC, ticket and MCS locks, new schedulers, condition
variables, \etc Some of these features were initially added in the
sequential setting but later ported to the concurrent setting.  During
this development process, many of our certified layers (including
their implementation, their functional specification,
and the layer refinement proofs) have undergone many rounds of
modifications and extensions. \CTOS{} makes such evolution process
much easier. For example, all certified layers in the sequential
kernel can be directly ported to the concurrent setting if they do not
use any synchronization. We have also merged the work by
\citet{chen16} on the interruptible kernel with device drivers using
our multicore model.

Overall, our certified \mCTOS{} kernel consists of 6500 lines of C and
...
redundant and semi-automatically generated,\ignore{ (or copied and
  pasted),} which makes our proof a little verbose.  For example, many
...
%%%%%%%%%%%
\ignore{
\ronghui{In my opinion, this paragraph is not that interesting.
The data like working hours still cannot convince people like Gernot.
Why not shrink it to leave some space?}
}
%%%%%%%%%%%
...
%%%%%%%%%%%
\ignore{
Assuming we work 250
days per year, this comes to roughly 400 lines of code per
...
multiple person weeks to prove that it always terminates.
}
...
Other than the FIFOBBQ  bug, we have also
found a few other bugs during verification.
Our initial ticket-lock implementation
contains a particularly subtle bug:
the spinning loop body (line 10 in Fig.~\ref{fig:exp:ticket_lock})
was implemented as
\lstinline$while($$\intp$\lstinline$L[i].now<t){}$. 
This passed all our tests, but  
...
cause an overflow (line 9 in  Fig.~\ref{fig:exp:ticket_lock}) and
the returned ticket
...
\ignore{
\paragraph{Extension and adaptation}
...
To reduce contention, all shared objects in {\mCTOS} are carefully
designed and pre-allocated with a fine-grained lock.
\ignore{
For example,
the {\mCTOS} process scheduler on each CPU does not need to access any
...
\begin{figure}\centering
%	\hspace{-.2cm}
	\includegraphics[width=3.125in]{figs/speedup_pb.pdf}
%	\hspace{-.2cm}
	\caption{Speedup of the throughput of \mCTOS{} in 2 and 3 cores against various server payloads (0 - 10,000) and IPC message sizes (0 - 20).}
	\label{fig:speedup_pb}
\end{figure}

Figure~\ref{fig:speedup_pb} shows the speedup of {\mCTOS} against various
...
%%%%%%%%
\ignore{Considering \mCTOS{} requires two more system calls than
  seL4,\mCTOS{}'s IPC is slightly slower than seL4's.}
%%%%%%%%
...
%%%%%%%%
\ignore{The IPC performance of \mCTOS{} is still practical. Verification not
...
\ignore{
	On {\mCTOS}, the performance of inter-core IPC is worse than that of
...
\begin{figure}\centering
	\hspace{-.2cm}
	\includegraphics[width=3.125in]{figs/ipc.pdf}
	\hspace{-.2cm}
...
on a USB stick. We use the standard 4KB pages in every setting~--- huge pages are not used.
 ...
 \ignore{Although there is an unverified branch of seL4 that supports
   ...
   performance of {\mCTOS} and an \emph{unverified} x86 version of
   seL4.}
...
show that 34 out of 60 benchmarks of \mCTOS{} are very close to the bare
metal (within the range of 5\%), and the others are listed in the Table \hao{we need a talbe?}. These
results shows that there is no significant overhead of \mCTOS{}.
	}
%%%%%%%%%
% In Figure~\ref{fig:eval_macro}, w
%%%%%%%%%
We normalize the running times of the benchmarks using the bare metal
performance as a baseline (100\%). The overhead of \mCTOS{} is
moderate and comparable to KVM. In some cases, \mCTOS{} performs
...
\mCTOS{} performs worse. This is because \mCTOS{} expose the raw disk
...
\ignore{
This is due to the idiosyncrasies of
the VirtIO interface~\cite{russell08}. 
In \mCTOS{}, we use the internal SSD driver
(instead of doing the pass-through) and  
we expose the raw disk interface
to the guest via VirtIO;
when there is a disk I/O request in the \texttt{vring},
\mCTOS{} immediately pulls it and issues a disk command to handle that
request, polling until the command finishes.
This leads to problems
with the guest Linux VirtIO driver, since when the guest Linux uses
VirtIO, it always sends disk I/O requests of length 4KB.
}
...
\ignore{
The VM monitor in \mCTOS{} is running as a user
process in ring 3 mode. This leads to a smaller kernel
...
process the exit. Therefore, there may be an impact on performance,
especially for those guest programs that frequently cause VM exits,
such as web servers generating frequent network-related external
