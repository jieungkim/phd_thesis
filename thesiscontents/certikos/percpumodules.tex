...
Per-CPU modules in $\certikos$ refined part of $\certikos$ modules--which are included in $\certikos_{\codeinmath{cpu}}$ in the figure, based on the $\mmcsbootfull$ layer-- 
...
thus, all operations on the lock object specify the lock ID as their argument, which is $lkid$ in this example.
...
use the the atomic fetch-and-increment (FAI) operation, that is implemented using the $\codeinmath{xaddl}$ instruction with the 
...
Based on them, who verify interruptible codes and insert them into the sequential $\certikos$~\cite{deepspec}, a prior work of our concurrent $\certikos$. 
...
which supports a container for resource management, two-level page tables, and dynamic page allocations for users.
...
\end{figure}
The container manages the resource that each thread (its children) can facilitate.
In the case of memory management, for instance,
the container memorizes the maximum number of pages (via the thread's quota) 
...
This also prevents the undesirable information channel between different threads that occurs due to such an attack.
...
In the figure, \lstinline$RData$ is a type of abstract data, and $AC$ is a container field in the abstract data. 
...
thus, threads that hold the lock for other shared objects 
...
\end{figure}
In detail, the page table is a partial map from a thread ID to its page table, as the definition related to the page table (written in $\coq$) is in
...
When any threads fails during this evaluation, it returns the magic number as its result, which is distinguishable from 
...
similar to the two-level in-kernel page-table structure but  with two additional levels, 
...
Those layers introducing VMCS and VMX objects, which consists of their data structures as well as primitives. 
...
\end{figure}
Our thread management first introduces the context switching written in Assembly, of which the implementation is in Figure~\ref{fig:chapter:certikos:kernel-context-switch-impl}.
...
the context switching happens between users and the kernel (via trap and system calls.)
Similar to the case of kernel context switching, $\certikos$ models the user context in the abstract data and verifies the functionality of user context switching. 
However, it differs from  kernel context switching.
...
The thread management and  CV (via sleep and wake-up) in our kernel heavily rely on this abstract queue library. 
...
The ready queue is private to each CPU, so, operations on the ready queue are only allowed by the owner of the queue.
On the other hand, other CPUs in the system can insert threads to the current CPU’s pending queue. 
The $\certikos$ kernel also provides a set of sleeping queues, which are shared among all CPUs.
...
Like what our layers do for other shared primitives, $\certikos$ provides the layer interface ($\PBThrd$)
...
We have introduced 60 layers for all those modules in our per-CPU layers, from the $\mmcsbootfull$ layer to the 
$\PBThrd$ layer.
