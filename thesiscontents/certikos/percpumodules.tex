...
which are included in $\certikos_{\codeinmath{cpu}}$ in the figure, based on the $\mmcsbootfull$ layer, 
into the local layer interface, $\PBThrd$.
Those modules consist of six components;
two spinlock modules, a device driver module, a memory management module,  a thread management module, and a virtual machine management module. 
...
This Quota works as a protection for a denial-of-service attack, where one thread repeatedly allocates pages and uses up all available memory. 
...
It also prevents the undesirable information channel between different threads that occur due to such an attack.

This container is a tree structure, and each CPU contains their tree with the root thread for each core. 
...
It reduces the complexity of our permission resolution because when each thread traps into the kernel,
...
The page table structure, \lstinline$PMapPool$, is a partial map from a thread identifier to its 2-level page table , \lstinline$PMap$.
...
When either of any threads fails during this evaluation, it returns the magic number as its result, which is distinguishable with 
...
Function local_enqueue_spec (n i: Z) (adt: RData): option RData :=
  ...
  <@$\cdots$@>
\end{lstlisting}
Since those queues can be shared by multiple threads in the system, 
...
and then create the child by giving the new thread ID to the child. 
...
As seen in these scheduling procedures, we do not allow the thread migration from one CPU to others.
