...
\jieung{I do not need to expose all the details (including specifications for send and receive functions / log replay functions, and so on.  But, if I have enough time to do that, I'll add more details in here}
...
\jieung{Do we need to mention about linking with CertiKOS in here? - If I have a spare time?}	
...
\jieung{adding the full layer diagram and explanations about it?}
\jieung{Don't we need to add evaluation? I do not want to include them because my thesis is not focusing on evaluations or performances at all}
