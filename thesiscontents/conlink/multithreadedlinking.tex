...
Figure~\ref{fig:chapter:linking:multithreaded-linking-structure} shows the overall structure and the abstract definitions that the structure relies on, and the number shows the places that we deal with the challenges we have mentioned above.

\subsubsection{$\CSched$ layer with $\lasmmach$} 

The figure shows that the bottom level of mulithreaded linking is 
a layer $\CSched$ associated with the focused CPU $cid$ and the relevant environmental context $\oracle_{cpuid}$, 
which uses the machine model $\lasmmach$, 
the machine model for $\compcertx$. 
The minimum restriction on the layer $\CSched$ is necessary, 
which is that the layer has to contain scheduling primitives to support software scheduler, thread sleep and yield.
Formally, 
...
when  first element ($lst$) is a local state for the CPU, which contains 
the register values ($\regs$), local copy of the memory ($mem$), as well as a private abstract data ($abs$),
...
and each primitive n the layer defines (including $\CSched$) its transition n by providing its specifications as follows:

\begin{center}
$\PLayer{\CSched}{cid}{\oracle_{\codeinmath{cid}}}(\Layer)(id)
...
m = m' \\
get\_curid(l) = cid\\
l' = \listcons{\listapp{l}{(\oracle_{\codeinmath{cid}}\ cid\  l)}}{(cid \ YIELD)}\\
get\_curid(l') = cid'\\
adt.kctxt[curid'] = \regs'\\
adt' = adt/[adt.kctxt := adt.kctxt/[cid = \regs]]}
{\PLayer{\CSched}{cid}{\oracle_{\codeinmath{cid}}}(\Layer)(\yield)
...
regarding scheduling and adding an $yield$ event.
...
Introducing the $\TLink$ with intermediate state machine models for multithreaded linking requires multiple steps. 
The first step is
...
Figur~\ref{fig:chapter:conlink:multithreaded-machine-model-easm} (a)
shows a simple overview how we divide a single state for $\lasmmach$ 
into a multiple thread states.
In this machine model, 
a set of 
A set of thread private states ($lst_{ti}$) is a partial map from a thread id to its private state designed as a tuple that contain
a register set, as well as an abstract state that may have a different type of an abstract state of $state_{\lasm}$. 
We, however, do not explicitly divide the a per-CPU memory into the multiple set of memories since 
it adds the complexity in the machine definition as well as providing the 
refinement theorem between machine models. 
Instead of it, the framework sues the memory model 
that guarantees the composition if we shows the mutual exclusion of each thread's memory usage, which is an algebraic memory model.
\jieung{I can copy and paste the related part of CCAL paper, but I do not know whether I will do or not. I'll live the explanation about this part before deciding it}
With this the new state definition, 
...
into  the scheduling evaluation that simply change the currently running thread ID and 
...
we provide per-thread interface and  Figure~\ref{fig:chapter:conlink:per-thread-machine-model} shows 
the key feature of our per-thread machine model.
...
During the remaining parts of this section, 
we provides the definitions that we presented in this section with the associated $\coq$ code. 
