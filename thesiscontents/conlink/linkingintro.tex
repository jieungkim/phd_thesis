...
We propose  $\ccalname$ in Chapter~\ref{chapter:ccal} and show the example of its usage, the MCS Lock verification, in Chapter~\ref{chapter:mcs-lock}.
...
Section~\ref{chapter:ccal:subsec:linking}
 informally mention that providing a formal  parallel composition rule more than two disjoint focused sets is possible.
We, however, do not provide any formal rules and theorems for this parallel composition in the chapter.
...
This linking process has a considerable cost due to multiple reasons.
First, concurrent linking requires machine syntax and semantics 
for the whole program and for partial concurrent programs (or the machine that can run both) as well.
...
written in both C and assembly via using the variance of the verified compiler $\compcert$ as well as its linking library described in Section~\ref{chapter:ccal:subsec:linking}. 
$\compcertx$, however, only includes its own local state (\ie, per-CPU or per-thread) as its machine state and achieves interleaved behaviors of concurrent programs via environmental contexts which contain the abstracted view of of other components in the system.
This isolation also has advantages in terms of the re-use of the verified compiler as well as efficiency in building  concurrent programs without directly handling complex interactions, but is not 
suitable to use $\compcertx$ for parallel composition.
In this sense, providing the parallel  composition requires us to 
 give the machine model that runs multiple instances of programs (\ie, a multicore machine model and a multithreaded machine model)
...
on the program implementation (such as software scheduler implementation or process creation),
 its logical environment (such as number of threads and the constraint on threads),
 or physical environment (such as number of CPUs as well as the hardware platform.)
...
Especially when dealing with process creation (or thread spawn) in the system that allows dynamic resource allocation,
a process creation usually allocates the maximum resources for the created process (which are typically determined by their parent process) and also set a initial program counter for the newly created process when it has been created. 
Instead of allowing local state updates from other threads, which raise the contradiction of the local state definition, 
our linking framework needs a way to calculate the proper side-effect on each thread' local state from shared data updates when it is necessary. 
In case of process creation, each thread requires it to set its proper dynamically allocated values when it first starts its evaluation,
the time which can be quite later than the time when it has been spawned. 
...
We also apply them to the linking techniques to the existing verified concurrent operating systems. 
...
The proof, however, does not consider the program that runs with the whole CPUs in the system, thus it is a partial correctness statement. 
To guarantee the full correctness of the program,
providing the judgment that connects the program runs with the full concurrent machine with the program on this local layer interface. 
...
For any program $ \codeinmath{P}$, our MCS Lock implementation $\codeinmath{M}_{\codeinmath{mcslock}}$, a full core set on the system ($\coreset$), a valid environmental context for CPU  $cid$ in $\coreset$ ($\oracle^{\codeinmath{mcs}}_{cid}$),  we says the concurrent linking for MCS Lock is 
\begin{center}
...
Definition~\ref{definition:conlink:con-linking-for-mcs-lock} provides the evidence about the formal connection between a program runs with a single local machine $\compcertx$ with a proper environmental context, and a program runs on top of a multicore machine model $\hardwarestepkwd$ with all CPUs.
%
%
...
Generalizing the theorem is available with any arbitrary layers, even with any concurrent and local machine models with satisfying desired properties,
and it is in 
...
\end{definition}
{\noindent}which we call it concurrent linking. 
Besides that, proofs about the compositionality of our verification are highly desirable to show that multiple instances in it rely on  consistent assumptions that can be ruled out via other instances in the same system.  Formally, it is
...
$\codeinmath{mach}_{\codeinmath{local}}$ is a machine model that runs the layer with a singleton focused set (such as a machine model of $\compcertx$)
\end{definition}
{\noindent}and we call it parallel composition. 
...
%\end{mathpar}
Proofs for them is based on the compositionality and the consistency of multiple strategies 
in partial programs, which discussed in Section~\ref{chapter:ccal:sec:ccal-overview}.
