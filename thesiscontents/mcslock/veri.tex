...
(precisely, ``$\ltyp{\PLayer{\mmcsboot}{cid}{\oracle_{mcs}}}{R_{(\mmcslockintro, \mmcsboot)}}{\codeinmath{M}_{\mmcslockintro}}{\PLayer{\mmcslockintro}{cid}{\oracle^{mcs}_{cid}}}$'' \\
...
\begin{tabular}{P{0.95\textwidth}}
    $\oracle^{mcs}_{cid}:\ \codeinmath{Z}\ \rightarrow\ \codeinmath{list event}\ \rightarrow\ \codeinmath{list event}.$\\
\end{tabular}\newline
...
queries the the environment context $\oracle^{mcs}_{cid}$, replays the log to see if if $\codeinmath{busy}$ is now $\bfalse$ and appends a $\GETBUSY$ event.
