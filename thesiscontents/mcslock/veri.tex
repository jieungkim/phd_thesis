...
%imposes some structure on the specifications~\cite{ccal16}.  
%Each
%record type $A$ must include at least a \emph{log of events} (written
%$l$) and a \emph{concurrent context} (written $\oracle$, further
%explained in Section~\ref{subsec:abstractoperationlayer}). For almost
...
%Because all CPUs see a single linear log,
...
$\codeinmath{mcs$\_$set$\_$next}$, and $\codeinmath{mcs$\_$get$\_$busy}$) from the \code{MMCSLockAbsIntro} layer.

The layers \code{MCSMCurID} through \code{MMCSLockAbsIntro}
...
\code{wait$\_$lock}), i.e. each layer ascribes a different
...
We first describe the first and the lowest tuple in our proofs, ($L_0$, $\codeinmath{M}$, $L_1$).
...
For example, \code{mcs$\_$SET$\_$NEXT}, one function in $\codeinmath{M}$, writes
...
The interface \code{MMCSLockIntro} contains the high level specification for each function defined in $\code{M}$. 
...
In $L_1$, we define \code{CalMCSLock}, which has the following type:
...
As a part of $R$, we define \code{match$\_$MCSLOCK} as follows:


\begin{definition}[\code{match$\_$MCSLOCK}]
Suppose that `loc' is among the proper field accessors for the MCS Lock (i.e. `\code{last}', `\code{ndpool[i].next}', or  `\code{ndpool[i].busy}' when `$0 \leq i <$  \code{TOTAL$\_$CPU}'). And, assuming that `\code{lk\_id}' is a lock identifier satisfies `$0 \leq$ \code{lk\_id} $<$ \code{lock$\_$range}' and \code{l} is a shared log. Then define 
...
\begin{theorem}[Simulation for $\codeinmath{{mcs}}\_\codeinmath{{SET}}\_\codeinmath{{NEXT}}$]
    \label{thm:chapter:mcslock:machine-state-refinement} Let $R$ be the relation defined as \code{match$\_$MCSLOCK}
...
\end{theorem}
 ...
 3) and, $A_0$ and $A_1$ are abstract datum for $L_0$ and $L_1$, respectively.
     With the given $R$, defined as \code{match\_MCSLOCK} 
     over \code{MCS$\_$LOC@}$mem_0$ and \code{LOG@}$A_1$, 
...
% ``\code{Z} $\rightarrow$ \code{list event} $\rightarrow$ \code{list event}''.
$\oracle:$ \code{Z -> list event -> list event}.
...
Similar to Section~\ref{chapter:mcslock:subsec:lowestmachinemodel}, we provide primitives in $L_0$ which query $\oracle$ and extend the log.
...
queries the the environment context $\oracle$, replays the log to see if if \code{busy} is now \code{false}, and appends a \code{GET\_BUSY} event.
...
%We named them as \code{mcs$\_$wait$\_$lock} and \code{mcs$\_$pass$\_$lock}.
...
%
%The \code{CalPassLockLoopTime} is the constant number for busy waiting termination proofs of  \code{mcs$\_$pass$\_$lock}. 
%Note that there are no arguments for the number because the loop in the  \code{mcs$\_$pass$\_$lock} is not relevant with other CPUs bound numbers. 
%
%The \code{CalPassLockLoopTime} is the constant number for busy waiting termination proofs of  \code{mcs$\_$pass$\_$lock}. 
%Note that there are no arguments for the number because the loop in the  \code{mcs$\_$pass$\_$lock} is not relevant with other CPUs bound numbers. 
%
%The \code{CalWaitLockTime} is the number for the termination proofs of \code{mcs$\_$wait$\_$lock}. 
%When we look at the \code{CalWaitLockTime} definition we need to calculate the number using \code{tq} which is the list of bound numbers for client codes that are waiting locks.
...
The meaning of a tuple \code{c1, c2, b, q, slow, t} is as follows: \code{c1} and \code{c2} are upper bounds on how many more operations 
...
\code{b} is either  \code{LHOLD} or \code{LFREE}, 
the lock status of the head of the queue.
\code{q} is the list of the CPUs currently waiting for the lock, 
and \code{t} is the list of bound numbers that 
corresponds to each element in \code{q}.
\code{slow} is a finite set which represents the subset of CPUs in \code{q} that have not yet executed their \emph{set next} operation.  
Our liveness proof is based on the fact that each CPU only needs to wait for CPUs that are ahead of it in \code{q}.

Some of this information is implicit in the state of the memory, while some of it (for example \code{c1} and \code{c2}) is purely ghost state. But in any case, it can be deduced from the complete history of events in the system, which is what the replay function \code{QS$\_$calLock} does. We define it by recursion on the list $l$, computing the new state after each event. A few representative cases of the function are shown in Figure~\ref{fig:chapter:mcslock:QS_CalLock}.  For example, the event
\code{SET$\_$BUSY} indicates that a thread releases the lock. If the CPU $i$ is already the  front of the queue $q$, it currently holds the lock (\code{LHOLD}), and the bound \code{c2} has not yet reached zero, and $i$ is not slow, then generating this event will reset the lock status to \code{LEMPTY} and remove the head element ($i$) from \code{q} and \code{t}. In any of those side conditions are not satisfied, on the other hand, the replay function is undefined (\code{None}). Similar
...
ordering, we need to track exactly what the valid state transitions are. So inside the ghost state layer, we also introduce a different relation  \code{Q$\_$CalMCSLock} which is mostly the same as \code{QS$\_$CalLock} but written as an (functional) inductive relation in Coq instead of a recursive function, and which has even more preconditions for when it is defined. We then add one more condition in the layer invariant saying that \code{Q$\_$CalMCSLock} and \code{QS$\_$CalLock} output the same
result. Most of the proofs inside the ghost layer are done using the relation instead of the function. For simplicity, we will ignore the distinction in the rest of the paper, and write the lemma statements about \code{QS$\_$CalLock} even if they used \code{Q$\_$CalMCSLock} in the actual Coq code.
...
If \code{CalMCSLock l = Some (tl, la, tq)} and \code{QS$\_$CalLock = Some (c1,c2,q,s,t)}, then \code{tl} is \code{NULL} if \code{q = $\nil$}, and \code{tl} the last element of \code{q} if \code{q $\neq \nil$}.
\end{lemma}

\begin{lemma}[next-correctness]
Let's assume that \code{CalMCSLock l = Some (tl, la, tq)} and \\
\code{QS$\_$CalLock = Some (c1,c2,q$\_$1++ $i$ :: $j$ ::q$\_$2,s,t)}, 
 then\\
  \code{lock$\_$array[$i$] = ($\_$, TOTAL$\_$CPU)}
  if \code{$j \in $ s}, and 
\code{lock$\_$array[$i$] = ($\_$,$j$)} if \code{$j \not\in$ s}.
\end{lemma}
...
Suppose \code{d} satisfies the invariant and
\code{wait$\_$qslock$\_$spec(d)= Some(d')}.\\
 Then \code{mcs$\_$acquire$\_$spec(d)= Some(d')}.
\end{theorem}
...
or owned by a CPU (\code{LockOwn}). The \code{lock} field is
...
execution of the system generates the log \code{l$_L$}.  A normal simulation
...
($R$ \code{l$_H$}, \code{$_L$}). Similarly, the local proof for CPU 2 would say there
exists some log \code{l'$_H$}. But in order to derive a simulation for the
entire system, we need the constraint that that \code{l$_H$} is equal to
\code{l'$_H$}. Our solution is to require the relation $R$ to be a function $f$. 
...
execute (\code{CalBound : Z $->$ MultiLog $->$ nat}).
...
in the waiting queue (\code{$j \in$ q}), then \code{CalBound $j$ l $>$ 0}. 
We then define a natural-number valued termination measure \code{$M_i$(c1,c2,h,q,s,l)}.  
...
\code{$|$q$_1|$=$|$t$_1|$} as well as \code{QS$\_$CalLock(e$_j$::l) =Some(c1',c2',h',q',s',t')} \\
for some $j\neq i$ and \code{CalBound(e$_j$::l) $>$ 0} .
Then we can split \code{q' = q$_{1}$'++$i$::q$_{2}$'}, and\\
\code{$M_i$(c1',c2',h',q$_{1}$',s',t$_{1}$',e$_j$::l)$< M_i$(c1,c2,h,q$_1$,s,t$_1$,l)}.
\end{lemma}
...
\code{$|$q$_1| = |$t$_1|$}, with \code{$i \not\in$ q$_1$} and \code{$i \not\in$s} and suppose $\oracle$ is a valid
context. If \code{$k$ $> M_i$(c1,c2,h,q$_1$,s,t$_1$)}, then there exists \code{l'} such
...
so that \code{CalWaitLockTime(t)} is greater than $\codeinmath{M}$ at that
