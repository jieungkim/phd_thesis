...
Eight of the primitives in $\mmcsbootfull$  are closely related to the $\mcsname$ Lock verification:
...
\end{tabular}\newline
when the first argument is a lock ID, the second argument is a CPU ID, and the third is the current log\footnote{Similar to a global log, we divide a single concurrent context into multiple ones for optimization; thus the first parameter (lock ID) is necessary for querying the proper concurrent context. We show how this set of concurrent contexts is connected with a single concurrent context which is discussed in Chapter~\ref{chapter:ccal} in the next Chapter (in Chapter~\ref{chapter:linking}.)} 
...
queries the the environment context $\oracle^{mcs(lkid)}_{\mathrm{i}}$ (\ie,  $lkid$ is a lock identifier and $\mathrm{i}$ is a CPU ID), replays the log to see if if $\codeinmath{busy}$ is now $\bfalse$ and appends a $\GETBUSY$ event.
...
relation in the refinement is just equality (including the relation for the global log and the environmental context ($\oracle^{mcs}_{cid}$) -- \ie, the relation for the strategies in the $\oracle^{mcs}_{cid}$), 
