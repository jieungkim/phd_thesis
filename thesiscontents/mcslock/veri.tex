\section{Events, logs, and concurrent contexts for MCS Lock}
...
In addition, the ``busy'' corresponds to the result of the \code{CAS} operation in Fig.~\ref{fig:exp:mcs_lock}.
...
For example, \code{mcs$\_$SET$\_$NEXT}, one function in $M$, writes
...
The interface \code{MMCSLockIntro} contains the high level specification for each function defined in $M$. 
