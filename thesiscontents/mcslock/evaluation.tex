...
Our verified MCS Lock is actually used by multiple clients in CertiKOS system. Figure~\ref{fig:palloc-example} (left) is one example of client code. Each client code that uses the lock defines its own lock wrappers. In the example, $\codeinmath{acquire\_lock\_AT}$ and $\codeinmath{release\_lock\_AT}$ are the lock primitives that are defined using the atomic lock specifications ($\codeinmath{wait\_hlock\_spec}$ and $\codeinmath{pass\_hlock\_spec}$, respectively), and they are only designed to the page allocation table protection. 
...
Those events may be different in the different layers because of the similar reason in Section~\ref{sec:liveness-atomicity}. Figure~\ref{fig:palloc-example} (right) shows the events that are for $\codeinmath{palloc}$, and we can refine logs that are using different types of shared memory events using the approach that we mention in Section~\ref{sec:verification}.

\ignore{Our MCS Lock verification is actually used with several client codes in CeritKOS system, and we briefly discuss how our MCS Lock can be associated with them by using the simplest example, $\codeinmath{palloc}$ in Figure~\ref{fig:palloc-example}.}
...
In this sense, we have introduced $\codeinmath{release\_shared}$, 
a logical primitive in the lowest layer in our system.
The primitive will add $\codeinmath{OMEME (l: list Integers.Byte.int)}$ event to the log, 
...
Thus, mapping this $\codeinmath{release\_shared}$ primitive with 
our MCS Lock verification is the second step to use the verified lock code with client codes.
After introducing the atomic lock specification ($\codeinmath{wait\_hlock\_spec}$ 
and $\codeinmath{pass\_hlock\_spec}$), we now can define lock acquire and release 
functions for each shared memory operation. 
To define $\codeinmath{release\_lock\_AT\_spec}$, 
the release lock specification designated to the page allocation table, 
we simply combine $\codeinmath{release\_shared}$ and $\codeinmath{pass\_hlock\_spec}$, and refine the general 
purpose shared memory event to the allocation table type event ($\codeinmath{OATE (a: ATable)}$).}

\ignore{Then, using this lock in  $\codeinmath{palloc}$ function and verifying the correctness of the code is simple. 
We only need to encapsulate the  $\codeinmath{palloc}$ code with $\codeinmath{acquire\_lock\_AT}$, 
the acquire lock specification for the page allocation table,
and $\codeinmath{release\_lock\_AT}$ primitives to protect the operation on the allocation table. 
Thanks to the proofs that we have done in MCS Lock module, our $\codeinmath{palloc}$ proof also contains the mutual exclusiveness, linearizability, liveness, and functional correctness.
