...
\jieung{From this section, I have reused many figures and texts (bur rewrite most of them) of our CCAL paper. If it is not valid, I have to re-build the following texts later}
...
Their local behaviors are same, calling the \code{lkclient} function in the layer $L_2$ in their bodies. 
...
components to build the operational semantics of a concurrent program \code{P} 
in a generic way
...
every edge/.style={draw, thick}]
    \path [->] (A) edge (B);
    \path [->] (B) edge node[above] {$?l, !i.\incticket, \return t\_n$} (C);
    \path [->] (C) edge [loop right] node[right] {$?(l \cons i.\incticket),!\emptye$} (C);
\end{scope}
\end{tikzpicture}
\end{center}%
...
scheduler strategy, $\sem{L[A]}{\codeinmath{P}}$ (the execution of \code{P} over $L[A]$) is
deterministic thanks to the strategy of the scheduler; 
the concurrent machine will run \code{P} when the control is
