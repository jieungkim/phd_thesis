Fixpoint <@$\replay_{\comm{shared}}$@> (l: Log) (b: Loc) (c: Id) :=
  match l with
    | nil => ret (<@$\vundef$@>, free) (* initial value *)
    | e :: l' =>  (* l' <@$\color{mygreen}\cons$@> a *)
      do r <- <@$\replay_{\comm{shared}}$@> l' b; (* Haskell syntax sugar*)
        match r, e with
